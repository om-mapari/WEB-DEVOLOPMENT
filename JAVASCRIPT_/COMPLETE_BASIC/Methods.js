...
// forEach method
const games = ['footbool','basebool','cricket','car']
const hi = games.forEach(game=>{
    console.log(game);
})
...
const res3 = []
names.forEach(name=>{
    if(name.length>3) res3.push(name)
})
console.log(res3);

// .some()  to check exist for just one
scores1 = [22,42,21,33]
scores2 = [22,42,21,52]
...
// .every() to check for everyone
console.log(scores1.every(score=> score<50));
console.log(scores2.every(score=> score<50));

// .split()
const sentence = 'I will have what she is haveing'
...
// Method Add all
myarr = [1,2,3,4]
const total = myarr.reduce((accumulator,curr)=>accumulator+curr)
console.log(total);
